<%@ Page Title="" Language="C#" MasterPageFile="~/ConsoleView/Site1.Master" AutoEventWireup="true" CodeBehind="UpHomeContent.aspx.cs" Inherits="OfficialWebsite.ConsoleView.UpHomeContent" %>
    ...
    <div class="page-content">
        ...
        <div class="row">
            <div class="col-xs-12 col-md-7">
                ...
                <div class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        <label class="col-sm-5 control-label no-padding-right" for="form-field-1">标记 </label>

                        <div class="col-sm-7">
                            <div class="clearfix">
                                <input type="text" id="tag" name="tag" placeholder="标记如：01" class="col-xs-10 col-sm-10" />
                            </div>
                        </div>
                    </div>
                    <div class="space-4"></div>
                    <div class="form-group">
                        <label class="col-sm-5 control-label no-padding-right" for="form-field-1">第一行 </label>

                        <div class="col-sm-7">
                            <input type="text" id="oneline" name="oneline" placeholder="第一行" class="col-xs-10 col-sm-10" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-5 control-label no-padding-right" for="form-field-1">第二行 </label>

                        <div class="col-sm-7">
                            <input type="text" id="twoline" name="twoline" placeholder="第二行" class="col-xs-10 col-sm-10" />
                        </div>
                    </div>
            ...
            <div class="col-xs-12 col-md-5">
                ...
                <div class="col-md-12">
                    <input type="file" class="hidden" id="fileupimg" name="fileupimg" /><label class="text-danger">点击图像上传图片(图片大小550*650)</label>
                </div>
            </div>
            <!-- /.col -->
        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.Result == "yes") {
                            ...
                            $("#twoline").val(data.twoline);
                            $("#imgHeadPortrait").attr("src", data.img + "?t=" + Math.random());
                        }
                    },
                    beforeSend: function () {
                        //执行前发生
            ...
            $("#btnSubmit").click(function () {
                var tag = $("#tag").val();
                var oneline = $("#oneline").val();
                var twoline = $("#twoline").val();
                var ID = $('#cid').html();
                var img = $("#imgHeadPortrait").attr("src").substr(0, $("#imgHeadPortrait").attr("src").indexOf("?"));
                if (tag != "" && oneline != "" && twoline != "") {
                    $('.form-group').removeClass("has-error");
                    $.ajax({
                        type: "post",
                        url: "ashx/UpdateHomeContentHandler.ashx",
                        data: { tag: tag, oneline: oneline, twoline: twoline, img: img, ID: ID },
                        dataType: "json",
                        success: function (data) {
                            //alert(data.Result);
                            if (data.Result == "yes") {
